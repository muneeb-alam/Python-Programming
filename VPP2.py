...
def status_of_BatteryStorage(offers,needs):
    ...
    for offer in offers:
        if offer.name is 'BatteryStorage':
            ...
            elif ((pv.power>0.0 or kwk.power>0.0) and (pd.power<=0.0 and dsm.power<=0.0)) and offer.charging_priority is not 1:                
                offer.price=offer.charging_price
                del(offers[index])
                print 'Need\nCharging Price:',offer.charging_price,'\n'
            else:
...
def print_offers(offers):
    ...
    for offer in offers:
        if offer.name is 'BatteryStorage':
            print 'Name', offer.name,'Power',offer.power, 'Price', offer.price,'Usable capacity',offer.usable_capacity_in_kWh,'(current capacity in %)',offer.percentage_current_capacity,'DOD',offer.DOD,'Profit',offer.profit
        else:
...
PV_Power=[30,0,25,100]
KWK_Power=[20,20,150,5]
PD_Power=[1000,100,0,5]
DSM_Power=[0,0,0,0]
...
for t in range(int((hours*60)/15)):
    ...
    for need in needs:
        ...
        while n < (int(N)) and need.power>0.1 and iteration_var<(int(N+20)):
            if (battery.power >=0 and battery.percentage_current_capacity>=0.0) or (pv.power or kwk.power or Draw_out_Load.power):
                pass
            else:
